...
def get_estimated_weights(df):
    df_w = pd.DataFrame(columns=["t","p+","p-","tw","rt","rp"])
    for i in np.arange(len(df)):
        w_i = df.iloc[i]["w"]
        w_i=w_i[1:]
        w_i = w_i[:-1]
        w_u = np.fromstring(w_i, dtype=float, sep=' ')
        row = [w_u[0],w_u[1],w_u[2],w_u[3],w_u[4],w_u[5]]
        df_w.loc[i]=row
    return df_w
...
def plot_spiderchart(weights_bots, weights_users):
    ...
    features = [r'$t$', r'$p^{+}$', r'$p^{-}$', r'$tw$', r'$rt$', r'$rp$']
    ...
    plt.legend(fontsize=12, loc=(0.9, 0.9))
...
weights_bots = get_estimated_weights(df_bots)
weights_users = get_estimated_weights(df_users)
