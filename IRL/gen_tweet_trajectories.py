...
from logger_IRL import get_logger
...
def tweet_traj_next_reduced(df_total):
    ...
    if (df_total.loc[0].activity ==1):    # action: original tweet    
        ...
        state_ = [1,0,0]
        signal=0
    elif ((df_total.loc[0].activity ==6) or (df_total.loc[0].activity ==8)): # state: got a positive reply or mention
        action_features_0 = np.asarray([0,0,0])
        state_ = [0,1,0]
        signal=0
    elif ((df_total.loc[0].activity == 7)):  # state: got a negative reply
        action_features_0 = np.asarray([0,0,0])
        state_ = [0,0,1]
        signal = 0
    ...
    for t in np.arange(len(df_total))[1:]:   
        ...
        action = list(a_dict.keys())[list(a_dict.values()).index(list(action_t))]
        state_t = np.concatenate((state_, action_t), axis=0)
        state = list(s_dict.keys())[list(s_dict.values()).index(list(state_t))]
        c_dict[state]+=1
        ...
        if tw_t>0:
            ...
            logger.error("Error: tweet id is not found.")
        if df_total.loc[t].kind=="p":
            if df_total.loc[t].activity==5: # state: got a retweet
                rt_count_t=1
                state_ = np.asarray([rt_count_t,0,0])
            elif (df_total.loc[t].activity==6) or (df_total.loc[t].activity==8): # state: got a positive reply or mention
                rp_count_t=1
                state_ = np.asarray([0,rp_count_t,0])
            elif (df_total.loc[t].activity==7) or (df_total.loc[t].activity==9): # state: got a negative reply or mention
                rp_count_t=1
                state_ = np.asarray([0,0,rp_count_t])
        else:
            state_ = np.asarray([rt_count_t,rp_count_t,rp_count_t])
    ...
    logger.debug("Trajectories shape: %s", trajectories.shape)
