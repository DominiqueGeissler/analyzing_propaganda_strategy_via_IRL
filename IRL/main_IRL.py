...
def main():
    ...
    df_RT_a=pd.read_csv(f"..\\data\\{user_type}\\df_rt_a_{user_type}.csv") # retweets generated by bots (resp. users)
    df_rp_a=pd.read_csv(f"..\\data\\{user_type}\\df_rp_a_{user_type}.csv") # replies generated by bots (resp. users)
    df_tw_a=pd.read_csv(f"..\\data\\{user_type}\\df_tw_a_{user_type}.csv") # original tweets generated by bots (resp. users)
    df_m_a=pd.read_csv(f"..\\data\\{user_type}\\df_m_a_{user_type}.csv") # tweets with mentions generated by bots (resp. users)

    ### passively involved tweets
    df_RT_s=pd.read_csv(f"..\\data\\{user_type}\\df_rt_s_{user_type}.csv")  # retweets of bots' (resp. users) generated content
    df_rp_pro_s=pd.read_csv(f"..\\data\\{user_type}\\df_rp_pro_s_{user_type}.csv") # supporting replies to bots' (resp. users) generated content
    df_rp_con_s=pd.read_csv(f"..\\data\\{user_type}\\df_rp_con_s_{user_type}.csv") # opposing replies to bots' (resp. users) generated content
    df_m_s=pd.read_csv(f"..\\data\\{user_type}\\df_m_s_{user_type}.csv") # tweets where bots (resp. users) were mentioned

    ### actively involved tweets dataframe
    df_author = pd.concat([df_RT_a, df_rp_a, df_tw_a, df_m_a], ignore_index=True)
    df_author.reset_index(inplace = True)
    users_subset= pd.unique(df_author.screen_name)
    logger.info(f"Number of {user_type}: {len(users_subset)}")
    ...
    structure = (3, 3)
    l1 = l2 = 0
    logger.debug(f"IRL parameters: discount={discount}, epochs={epochs}, learning_rate={learning_rate}, structure={structure}, l1={l1}, l2={l2}")

    df_results = pd.DataFrame(columns = ['screen_name','r','w'])
    # df_trajectories = pd.DataFrame(columns = ['screen_name','trajectory'])
    # threshold parameter
    t_a = 5 # threshold to filter out bots (resp. users) that have not performed at least t_a sharing activities (original tweets +retweets + replies + mentions)
    t_p = 5 # threshold to filter out bots (resp. users) that have not received at least t_p feedback from other accounts (retweets + replies + mentions)
    logger.debug(f"Threshold parameters: t_a={t_a}, t_p={t_p}")

    count = 0
    logger.info(f"Start IRL for {user_type}")
    for user_name in users_subset:

        # creating user-based dataframe
        # states
        RT_p_u, n_rt_p = create_user_dataframe(df_RT_s, user_name, 'user_retweeted', 5, "p")  # state: got a retweet
        rp_pro_p_u, n_rp_pro_p = create_user_dataframe(df_rp_pro_s, user_name, 'in_reply_to_screen_name', 6,
                                                       "p")  # state: got a positive reply
        rp_con_p_u, n_rp_con_p = create_user_dataframe(df_rp_con_s, user_name, 'in_reply_to_screen_name', 7,
                                                       "p")  # state: got a negative reply
        m_p_u, n_m_p = create_user_dataframe(df_m_s, user_name, 'mention', 8, "p")  # state: got a mention
        n_p = n_rt_p + n_rp_pro_p + n_rp_con_p + n_m_p  # number of states
        ...
        if n_a> t_a and n_p> t_p: # filtering out some users
            logger.info(f"User {user_name} (actions: {n_a}, states: {n_p}) is being processed. This is user {count}.")
            # creating actions dataframe
            df_active_u = pd.concat([tw_a_u, RT_a_u, rp_a_u, m_a_u], ignore_index=True)
            ...
            rp_pro_p_u = adjust_reply_df(rp_pro_p_u)
            rp_con_p_u = adjust_reply_df(rp_con_p_u)
            m_p_u = adjust_mention_df(m_p_u)
            df_passive_u = pd.concat([RT_p_u, rp_pro_p_u, rp_con_p_u, m_p_u], ignore_index=True)
            ...
            w = np.linalg.lstsq(feature_matrix,r)[0] # compute weights of each feature
    ...
    logger.info(f"IRL finished for {count} {user_type}")

    df_results.to_csv(f"../data/df_results_{user_type}_IRL.csv",index=False) # save rewards in a csv file (NB:change the name of the file for generic users in df_results_users_IRL.csv)
