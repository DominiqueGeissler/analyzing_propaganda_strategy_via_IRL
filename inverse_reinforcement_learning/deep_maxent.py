"""
This file stems from the paper
"Detecting Troll Behavior via Inverse Reinforcement Learning: A Case Study of Russian Trolls in the 2016 US Election",
by Luca Luceri, Silvia Giordano, and Emilio Ferrara,
 which has been accepted in the Proceedings of the AAAI International Conference on Web and Social Media (ICWSM 2020),
 https://arxiv.org/pdf/2001.10570.pdf
 https://github.com/luceriluc/Detecting-Troll-Behavior-via-Inverse-Reinforcement-Learning

 The file has not been modified.


Implements deep maximum entropy inverse reinforcement learning based on Ziebart et al., 2008 and Wulfmeier et al., 2015.

Script developed by Matthew Alger, 2015
...
def irl(structure, feature_matrix, n_actions, discount, transition_probability,
    ...
    # Initialise alf weight, bet bias.
    if initialisation == "normal":
        alf = th.shared(rn.normal(size=(1, structure[-1])), name="alpha",
                      borrow=True)
    else:
        alf = th.shared(rn.uniform(size=(1, structure[-1])), name="alpha",
                      borrow=True)
    hist_alf_grad = T.zeros(alf.shape)  # For AdaGrad.
    ...
    for W, b in zip(weights, biases):
        ...
        # phis[1] = phi1 etc.
    # Reward.
    r = alf.dot(phis[-1]).reshape((n_states,))
    ...
    updates = []
    alf_grad = phis[-1].dot(svf - expected_svf).T
    hist_alf_grad += alf_grad**2
    adj_alf_grad = alf_grad/(adagrad_epsilon + T.sqrt(hist_alf_grad))
    updates.append((alf, alf + adj_alf_grad*learning_rate))
