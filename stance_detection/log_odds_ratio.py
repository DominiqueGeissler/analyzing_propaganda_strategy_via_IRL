...
from logger_stance import get_logger
import tqdm
import numpy as np
import pandas as pd
import argparse
...
path_output = "..\\data\\odds_ratio\\"
logger = get_logger("LogOddsRatio")
...
class LogOddsRatio:
    ...
    def __init__(self, corpus_i, corpus_j, name, background_corpus=None, lower_case=True, rm_stopwords=True,
        ...
        with open(str(path_output + "vocabs_i_" + name + ".txt"), "w", encoding="utf-8") as f:
            for k, v in self.y_i.items():
                f.write(f"{k},{v}\n")
        with open(str(path_output + "vocabs_j_" + name + ".txt"), "w", encoding="utf-8") as f:
            for k, v in self.y_j.items():
                f.write(f"{k},{v}\n")
        with open(str(path_output + "vocabs_alpha_" + name + ".txt"), "w", encoding="utf-8") as f:
        ...
        self.z_scores_reversed = {k: v for k, v in sorted(self.z_scores.items(), key=lambda item: item[1])}

        # Write to files as backup
        with open(str(path_output + "delta_" + name + ".txt"), "w", encoding="utf-8") as f:
            for k, v in self.delta.items():
                f.write(f"{k},{v}\n")
        with open(str(path_output + "sigma_2_" + name + ".txt"), "w", encoding="utf-8") as f:
            for k, v in self.sigma_2.items():
                f.write(f"{k},{v}\n")
        with open(str(path_output + "z_scores_" + name + ".txt"), "w", encoding="utf-8") as f:
...
def main():
    ...
    parser.add_argument("--save_top_words", default=10, type=int, required=False)
    ...
    corpus_i_name = args.filepath_corpus_i.split("\\")[-1].split(".")[0].replace("data_odds_", "")
    corpus_j_name = args.filepath_corpus_j.split("\\")[-1].split(".")[0].replace("data_odds_", "")

    # Read file into list of texts
    df_corpus_i = pd.read_csv(args.filepath_corpus_i)
    corpus_i = [text.strip() for text in df_corpus_i["text_normalized"]]
    del df_corpus_i
    df_corpus_j = pd.read_csv(args.filepath_corpus_j)
    corpus_j = [text.strip() for text in df_corpus_j["text_normalized"]]
    del df_corpus_j
    if args.filepath_background_corpus is not None:
        df_corpus_bg = pd.read_csv(args.filepath_background_corpus)
        corpus_bg = [text.strip() for text in df_corpus_bg["text_normalized"]]
        del df_corpus_bg
    else:
        corpus_bg = None
    ...
    log_odds_ratio = LogOddsRatio(
        corpus_i, corpus_j, corpus_i_name, corpus_bg, tokenizer=tweet_tokenizer)

    # Save top words into a file
    if args.save_top_words is not None and args.save_top_words > 0:
        ...
        with open(str(path_output + "top_words_" + corpus_i_name + ".txt"), "w", encoding="utf-8") as f:
            i = 0
        ...
        logger.info(f"Saving bottom {args.save_top_words} words ranked by Z-score")
        with open(str(path_output + "top_words_" + corpus_j_name + ".txt"), "w", encoding="utf-8") as f:
            i = 0
            for k, v in log_odds_ratio.z_scores_reversed.items():
                f.write(k + "\n")
                i += 1
                if i >= args.save_top_words:
                    break
...
# python log_odds_ratio.py --filepath_corpus_i ..\\data\\stance_data\\data_odds_support.csv --filepath_corpus_j ..\\data\\stance_data\\data_odds_oppose_neither.csv
# python log_odds_ratio.py --filepath_corpus_i ..\\data\\stance_data\\data_odds_oppose.csv --filepath_corpus_j ..\\data\\stance_data\\data_odds_support_neither.csv